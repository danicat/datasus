...
#' Provides Access to the Declarations of Death (Fetal) Dataset from the Mortality Information System (SIM).
#'
#' \code{sim.dofet} returns a data.frame with a subset of the Declarations of Death - Fetal (DOFET) dataset
#' 
#' @details
...
#'
#' pr10 <- sim.dofet(2010)
sim.dofet <- function(years, language = datasus.lang()) {
  sim.load("DOFET", years, language = language)
}